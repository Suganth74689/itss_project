...
import { FileText, Database, ShieldCheck, Layers, ChevronDown, Sparkles } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-slate-900 border-b border-slate-800 px-6 py-3.5 flex items-center justify-between sticky top-0 z-30 shadow-xl backdrop-blur-md">
        ...
        <div>
          <div className="flex items-center gap-2.5">
            <h1 className="text-xl font-display font-black tracking-tight text-white flex items-center gap-2">
              NEXUS <span className="bg-gradient-to-r from-blue-400 to-indigo-400 bg-clip-text text-transparent">BANKING AI</span>
            </h1>
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold bg-blue-500/10 text-blue-400 border border-blue-500/30 tracking-wider uppercase flex items-center gap-1">
              <Sparkles className="w-3 h-3 text-blue-400" /> Enterprise Engine
            </span>
          </div>
          <p className="text-[11px] text-slate-400 font-mono flex items-center gap-1.5 mt-0.5">
            <ShieldCheck className="w-3.5 h-3.5 text-blue-400" />
            Deterministic DuckDB Core Banking Retrieval • Zero-Hallucination Policy
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="hidden lg:flex items-center space-x-2 px-3.5 py-1.5 rounded-xl bg-slate-800/80 border border-slate-700 text-xs font-mono text-slate-300">
          <Database className="w-4 h-4 text-emerald-400 animate-pulse" />
          <span className="font-semibold text-slate-200">DuckDB Engine Active</span>
        </div>

        {/* Record Evidence Drawer Button */}
        <button
          onClick={onToggleEvidence}
          className={`flex items-center space-x-2 px-4 py-2 rounded-xl text-xs font-bold font-mono transition-all border shadow-lg ${
            showEvidence
